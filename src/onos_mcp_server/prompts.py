...
def register_prompts(mcp_server: FastMCP):
    ...
    def deploy_flow(deviceId: str, flowId: str, match: str, actions: str, priority: str = "default") -> str:
        """
    ...
    def debug_network_issue(issueDescription: str, deviceId: str = "") -> list[Message]:
        ...
        messages.append(AssistantMessage("I will help analyze the issue. "
                                          "Please provide any additional details (e.g., log excerpts or error codes)."))
        return messages

    @mcp_server.prompt("configure-qos-policy")
    def configure_qos_policy(deviceId: str, policyType: str, trafficClass: str, parameters: str) -> str:
        """
        ...
        treatment = policy_treatments.get(policyType, "applying custom QoS configuration")
    ...
    def optimize_qos_settings(optimizationGoal: str, deviceId: str = "") -> list[Message]:
        """
        ...
        if optimizationGoal not in valid_goals:
            return [
                ...
                AssistantMessage(f"The optimization goal must be one of: {', '.join(valid_goals)}. "
