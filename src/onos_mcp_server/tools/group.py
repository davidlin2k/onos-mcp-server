...
async def add_group(deviceId: str, group_type: str, app_id: str, app_cookie: str, buckets: List[Dict[str, Any]]) -> str:
...
async def remove_group(deviceId: str, appCookie: str) -> str:
    ...
    try:
        ...
        return f"Group with cookie {appCookie} removed successfully from device {deviceId}"
    except Exception as e:
        return f"Error removing group with cookie {appCookie} from device {deviceId}: {str(e)}"

async def add_buckets_to_group(deviceId: str, appCookie: str, buckets: List[Dict[str, Any]]) -> str:
    ...
    try:
        ...
        result = await make_onos_request("post", f"/groups/{deviceId}/{appCookie}/buckets", json=buckets_data)
...
async def remove_buckets_from_group(deviceId: str, appCookie: str, bucketIds: str) -> str:
    ...
    """
    try:
        await make_onos_request("delete", f"/groups/{deviceId}/{appCookie}/buckets/{bucketIds}")
