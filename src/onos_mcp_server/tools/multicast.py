...
async def add_multicast_route(group: str, source: str, sources: List[str], sinks: List[str]) -> str:
...
async def add_multicast_sink(group: str, source: str, sink: str) -> str:
    ...
    try:
        ...
        result = await make_onos_request("post", f"/mcast/sinks/{group}/{source}", json=sink_data)
        return f"Sink added successfully to multicast route from {source} to group {group}"
    except Exception as e:
