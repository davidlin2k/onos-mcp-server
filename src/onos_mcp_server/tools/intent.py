...
async def get_intents_by_application(appId: str, detail: bool = False) -> str:
    ...
    try:
        params = {"detail": str(detail).lower()}
        intents = await make_onos_request("get", f"/intents/application/{appId}", params=params)
        return str(intents)
    except Exception as e:
...
async def get_intent_installables(appId: str, key: str) -> str:
    ...
    """
    try:
        installables = await make_onos_request("get", f"/intents/installables/{appId}/{key}")
